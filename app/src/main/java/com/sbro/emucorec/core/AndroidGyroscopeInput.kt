...
class AndroidGyroscopeInput(
    ...
    companion object {
        ...
        fun isModeAvailable(context: Context, mode: Int): Boolean {
            ...
            return when (mode) {
                Ps3CoreConfig.GYRO_MODE_AIM ->
                    manager.getDefaultSensor(Sensor.TYPE_GYROSCOPE) != null
                Ps3CoreConfig.GYRO_MODE_STEERING ->
    ...
    private var mode = Ps3CoreConfig.GYRO_MODE_OFF
    ...
    override fun onSensorChanged(event: SensorEvent) {
        val validSample = when (mode) {
            Ps3CoreConfig.GYRO_MODE_AIM -> readAimValues(event)
            Ps3CoreConfig.GYRO_MODE_STEERING -> readSteeringValues(event)
            else -> false
        }
