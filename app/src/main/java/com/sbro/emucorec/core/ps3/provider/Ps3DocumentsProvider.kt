package com.sbro.emucorec.core.ps3.provider
...
class Ps3DocumentsProvider : DocumentsProvider() {
    private val root = "Ps3Root"
    ...
    private fun getStorageDir(): File {
        return context?.let { EmulatorStorage.ps3Root(it) }
    ...
    private fun applyCursor(cursor: MatrixCursor, file: File?, documentId: String?) {
        ...
        row.add(DocumentsContract.Document.COLUMN_DOCUMENT_ID, resolvedDocumentId)
            .add(DocumentsContract.Document.COLUMN_DISPLAY_NAME, if (isRoot) "EmuCoreC PS3" else resolvedFile.name)
    ...
    override fun queryRoots(projection: Array<out String>?): Cursor {
        ...
        cursor.newRow()
            ...
            .add(DocumentsContract.Root.COLUMN_TITLE, "EmuCoreC PS3")
