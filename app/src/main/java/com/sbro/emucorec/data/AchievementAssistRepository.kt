    ...
    private suspend fun request(
        ...
        val result = runCatching {
            val connection = (URL(WORKER_URL).openConnection() as HttpURLConnection).apply {
                ...
                setRequestProperty("X-EmuCoreC-Assist-Key", WORKER_ACCESS_KEY)
            }
        ...
        }.getOrElse { workerError ->
            if (action == "translate") {
                val translatedTitle = fallbackTranslate(languageTag, trophyName)
                val translatedBody = fallbackTranslate(languageTag, trophyDetail)
                if (translatedTitle.isNotBlank() || translatedBody.isNotBlank()) {
                    AchievementAssistResult(
                        title = translatedTitle.ifBlank { trophyName },
                        body = translatedBody.ifBlank { trophyDetail }
                    )
                } else {
                    throw workerError
                }
        ...
        result.also { writeCache(cacheKey, it) }
    }

    private fun fallbackTranslate(languageTag: String, text: String): String {
        if (text.isBlank()) return text
        return runCatching {
            val url = "https://translate.googleapis.com/translate_a/single?client=gtx&sl=auto&tl=" +
                URLEncoder.encode(languageTag, "UTF-8") + "&dt=t&q=" + URLEncoder.encode(text, "UTF-8")
            ...
            try {
                if (connection.responseCode in 200..299) {
                    ...
                    val sentences = root.optJSONArray(0) ?: return@runCatching text
                    buildString {
                        for (i in 0 until sentences.length()) {
                            sentences.optJSONArray(i)?.optString(0)?.let(::append)
                        }
                    }.ifBlank { text }
                } else text
            } finally {
