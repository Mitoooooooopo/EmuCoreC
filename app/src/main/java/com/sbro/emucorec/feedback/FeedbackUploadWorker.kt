...
class FeedbackUploadWorker(
    ...
    private fun upload(directory: File, feedback: QueuedFeedback): Int {
        ...
        val boundary = "EmuCoreC-${UUID.randomUUID()}"
        val connection = (endpoint.openConnection() as HttpURLConnection).apply {
            ...
            // Keep the shared Worker auth header for endpoint compatibility and identify this app separately.
            setRequestProperty("X-EmuCoreX-Key", BuildConfig.FEEDBACK_API_KEY)
            setRequestProperty("X-EmuCore-App", "EmuCoreC")
            setRequestProperty("User-Agent", "EmuCoreC/${BuildConfig.VERSION_NAME} (Android)")
        }
        try {
    ...
    private fun diagnostics(): Map<String, String> {
        return linkedMapOf(
            ...
            "appVersion" to "EmuCoreC ${BuildConfig.VERSION_NAME}",
            "appBuild" to BuildConfig.VERSION_CODE.toString(),
            "coreProject" to "RPCS3 core · RPCS3 upstream core",
            "coreVersion" to runCatching { net.rpcsx.RPCSX.instance.getVersion() }.getOrDefault("RPCS3 core"),
            "coreSource" to "https://github.com/RPCS3/rpcs3",
            "upstreamCore" to "https://github.com/RPCS3/rpcs3",
