...
class AppUpdateReleaseParserTest {
    ...
    fun `keeps regular and parallel APK assets separate`() {
        val assets = classifyAppUpdateAssets(
            listOf(
                AppUpdateAsset("EmuCoreC-v0.1.6.apk", "https://example.com/main.apk", 1000),
                AppUpdateAsset("EmuCoreC-v0.1.6-parallel.apk", "https://example.com/parallel.apk", 2000)
            )
        )
    ...
    fun `does not offer parallel download when release has only normal APK`() {
        val assets = classifyAppUpdateAssets(
            listOf(AppUpdateAsset("EmuCoreC-v0.1.5.apk", "https://example.com/main.apk", 1000))
        )
    ...
    fun `ignores non APK and unavailable parallel assets`() {
        val assets = classifyAppUpdateAssets(
            listOf(
                ...
                AppUpdateAsset("EmuCoreC-parallel.apk", null, 2000)
            )
        )
