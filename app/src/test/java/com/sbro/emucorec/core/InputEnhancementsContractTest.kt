...
class InputEnhancementsContractTest {
    ...
    fun defaultsAndPresetRangesMatchTheRuntimeContract() {
        ...
        assertEquals(Ps3CoreConfig.TOUCH_HAPTICS_PRESET_BALANCED, defaults.touchHapticsPreset)
        assertEquals(60, defaults.touchHapticsStrength)
        assertEquals(Ps3CoreConfig.GYRO_MODE_OFF, defaults.gyroMode)
        assertEquals(100, defaults.gyroSensitivity)
        assertEquals(45, defaults.gyroSmoothing)
        assertEquals(0, Ps3CoreConfig.TOUCH_HAPTICS_PRESET_SOFT)
        assertEquals(3, Ps3CoreConfig.TOUCH_HAPTICS_PRESET_STRONG)
        assertEquals(1, Ps3CoreConfig.GYRO_MODE_AIM)
        assertEquals(2, Ps3CoreConfig.GYRO_MODE_STEERING)
    }

    @Test
    fun globalBackupAndPerGameRepositoriesPersistEveryInputField() {
        val root = sourceRoot()
        val global = root.resolve("core/Ps3CoreConfigRepository.kt").readText()
        val backup = root.resolve("core/SettingsBackupRepository.kt").readText()
        val perGame = root.resolve("core/Ps3GameSettingsRepository.kt").readText()
        val fields = listOf(
            "touchHaptics",
            "touchHapticsPreset",
        ...
        fields.forEach { field ->
            assertTrue("$field must be included in the shared JSON encoder", ".put(\"$field\", $field)" in global)
            assertTrue("$field must be included in the shared JSON decoder", Regex("""(?m)^\s*$field\s*=""").containsMatchIn(global))
        }
        assertTrue("Backup must use the complete Android-control codec", "ensureDefaultsPersisted().toJsonObject()" in backup)
        assertTrue("Per-game profiles must use the complete Android-control codec", "effective.toJsonObject()" in perGame && "toPs3CoreConfig(base)" in perGame)
    }
    ...
    fun everySupportedLocaleContainsTheInputEnhancementStrings() {
        ...
        assertTrue("Expected the complete input string set", requiredKeys.size >= 20)
    ...
    private fun sourceRoot(): Path = appModule().resolve("src/main/java/com/sbro/emucorec")
